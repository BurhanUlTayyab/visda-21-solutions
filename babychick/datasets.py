...
class DataAugmentationForBEiT(object):
    def __init__(self, mean, std, IMG_SIZE, WINDOW_SIZE, NUM_PATCHES, MAX_MASK_PATCHES_PER_BLOCK, MIN_MASK_PATCHES_PER_BLOCK, DISCRETE_VAE_TYPE):
        self.common_transform = transforms.Compose([
            transforms.ColorJitter(0.4, 0.4, 0.4),
            transforms.RandomHorizontalFlip(p=0.5),
            RandomResizedCropAndInterpolationWithTwoPic(
                size=IMG_SIZE, second_size=112,
        ...
        if DISCRETE_VAE_TYPE == "dall-e":
            self.visual_token_transform = transforms.Compose([
                transforms.ToTensor(),
                map_pixels,
            ])
        elif DISCRETE_VAE_TYPE == "customized":
            self.visual_token_transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(
        ...
        self.masked_position_generator = MaskingGenerator(
            WINDOW_SIZE, num_masking_patches=NUM_PATCHES,
...
def build_beit_pretraining_dataset(args):
    ...
    transform = DataAugmentationForBEiT(mean, std, 224, WINDOW_SIZE, NUM_PATCHES, MAX_MASK_PATCHES_PER_BLOCK, MIN_MASK_PATCHES_PER_BLOCK, DISCRETE_VAE_TYPE)
